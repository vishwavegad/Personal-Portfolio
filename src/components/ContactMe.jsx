    ...
    <section id='contact' className="mt-20 scroll-mt-20" style={{color:'var(--text-color'}}>
      ...
      <form onSubmit={onSubmit}>
        <div className="flex flex-col justify-center items-center mt-10 bg-[var(--card-background-color)] rounded-xl mx-auto shadow-md w-full max-w-md md:max-w-lg lg:max-w-4xl">
          <input type="text" placeholder="Name" name='name' required className="border rounded-2xl p-2 w-60 md:w-100 m-4 mt-10"/>
          <input type="text" placeholder="Email" name='email' required className="border rounded-2xl p-2 w-60 md:w-100 m-4"/>
          <textarea name="message" placeholder="Message" className="border rounded-2xl p-2 h-50 w-60 md:w-100 m-4"></textarea>
          <button className='rounded-3xl p-2 w-60 md:w-100 mb-10 hover:cursor-pointer bg-[var(--btn-color)] text-[var(--btn-text-color)] hover:bg-[var(--btn-hover-color)]'>Submit</button>
        </div>
      </form>
    </section>
  )
