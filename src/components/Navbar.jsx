...
function Navbar() {
  ...
  return (
    <section className="fixed top-0 left-0 w-full h-12 z-50">
      <div
        className="relative w-full flex items-center justify-center px-4 h-full"
      ...
      >
        ...
        >
          {links.map(({ id, link, sectionId }) => (
            <li className="font-bold p-3 pb-3 cursor-pointer" key={id} onClick={()=>scrollToSection(id)}>
              {link}
            </li>
          ))}
        </ul>
        ...
        {nav && (
          <ul
            className="flex flex-col justify-between items-center absolute top-0 left-0 w-50 py-10 z-40 space-y-6 text-white"
            style={{ backgroundColor: "var(--dropdown-color)" }}
          >
            {links.map(({ id, link }) => (
